"""
研究编号: RS-20260622-107
论文标题: QuantAgent: Price-Driven Multi-Agent LLMs for High-Frequency Trading（扩展 003603）
研究主题: QuantAgent: Price-Driven Multi-Agent LLMs for High-Frequency Trading（扩展 003603）

数据位置:
  - MongoDB: mongodb://localhost:27017 / quant_db / daily_bars
  - 种子文献: /Users/derek/Documents/Github/paperwriterAI/data/seed_papers
  - 本研究档案: /Users/derek/Documents/Github/paperwriterAI/data/research/RS-20260622-107_QuantAgent_Price_Driven_Multi_Agent_LLMs
"""

MONGO_URI = 'mongodb://localhost:27017'
MONGO_DB = 'quant_db'
MONGO_COLLECTION = 'daily_bars'
RESEARCH_ID = 'RS-20260622-107'
...
def backtest_quantagent_vote(df, *, use_trend=True, use_meanrev=True, use_vol=True):
    df = df.copy()
    df["agent_trend"] = np.where(df["ma5"] > df["ma20"], 1, -1)
    df["agent_meanrev"] = np.select(
        [df["rsi"] < 30, df["rsi"] > 70],
        [1, -1],
        default=0,
    )
    df["agent_vol"] = np.select(
...
print("\n=== Step 3: QuantAgent 多智能体投票模拟 ===")
df_qa = backtest_quantagent_vote(df, use_trend=True, use_meanrev=True, use_vol=True)
qa_metrics = calculate_metrics(df_qa, "cum_quantagent")
qa_ablations = {
    "trend_only": calculate_metrics(backtest_quantagent_vote(df, use_trend=True, use_meanrev=False, use_vol=False), "cum_quantagent"),
    "meanrev_only": calculate_metrics(backtest_quantagent_vote(df, use_trend=False, use_meanrev=True, use_vol=False), "cum_quantagent"),
    "vol_only": calculate_metrics(backtest_quantagent_vote(df, use_trend=False, use_meanrev=False, use_vol=True), "cum_quantagent"),
    "trend_meanrev": calculate_metrics(backtest_quantagent_vote(df, use_trend=True, use_meanrev=True, use_vol=False), "cum_quantagent"),
    "trend_vol": calculate_metrics(backtest_quantagent_vote(df, use_trend=True, use_meanrev=False, use_vol=True), "cum_quantagent"),
    "meanrev_vol": calculate_metrics(backtest_quantagent_vote(df, use_trend=False, use_meanrev=True, use_vol=True), "cum_quantagent"),
}
print("\nQuantAgent(投票模拟) 回测结果:")
print(f"  总收益率: {qa_metrics['total_return']}%")
print(f"  年化收益率: {qa_metrics['annual_return']}%")
print(f"  夏普比率: {qa_metrics['sharpe_ratio']}")
print(f"  最大回撤: {qa_metrics['max_drawdown']}%")
print(f"  胜率: {qa_metrics['win_rate']}%")
print(f"  交易次数: {qa_metrics['total_trades']}")
...
all_results = {
    ...
    "quantagent": {
        ...
        "agents": [
            {"id": "trend", "signal": "ma5 > ma20 => +1 else -1"},
            {"id": "meanrev", "signal": "rsi<30 => +1; rsi>70 => -1; else 0"},
            {"id": "vol", "signal": "close<bb_lower => +1; close>bb_upper => -1; else 0"},
        ],
        "aggregator": "majority_vote_on_sign(sum(signals))",
        "metrics": qa_metrics,
        "ablations": qa_ablations,
    },
}
...
with open(qa_results_path, "w", encoding="utf-8") as f:
    json.dump(
        {
            "experiment_date": all_results["experiment_date"],
            "stock": all_results["stock"],
            "data_range": all_results["data_range"],
            "data_points": all_results["data_points"],
            "quantagent": all_results["quantagent"],
            "baseline_best_strategy": best_strategy,
            "baseline_best_strategy_metrics": all_results["strategies"][best_strategy]["metrics"],
...
qa_trace = df_qa[["date", "close", "return", "agent_trend", "agent_meanrev", "agent_vol", "agent_vote", "position", "strategy_return"]].tail(200).copy()
qa_trace["date"] = qa_trace["date"].astype(str)
...
with open(qa_trace_path, "w", encoding="utf-8") as f:
    json.dump(qa_trace.to_dict("records"), f, ensure_ascii=False, indent=2)
